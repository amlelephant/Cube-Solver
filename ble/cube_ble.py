...
# ── Windows BLE compatibility — MUST be before all other imports ──────────
import win_compat  # sets sys.coinit_flags = 0 before pythoncom can set STA
# ─────────────────────────────────────────────────────────────────────────
...
class CubeConnection:
    ...
    def _schedule_on_loop(self, callback):
        """Run callback on the asyncio loop thread, even if called from another thread."""
    ...
    def _on_notification(self, sender, data: bytearray):
        ...
        if msg_type == MSG_ROTATION:
            ...
            if state and self._state_future and not self._state_future.done():
                ...
                self._schedule_on_loop(resolve_state)
            ...
            if payload and self._battery_future and not self._battery_future.done():
                ...
                def resolve_battery():
                    if self._battery_future and not self._battery_future.done():
                        self._battery_future.set_result(level)

                self._schedule_on_loop(resolve_battery)
